...
public class Elevator extends SubsystemBase {
  ...
  public Elevator() {
    ...
    // Zeroing the selected sensor position (Robot must always turn on when the whole elevator is down)
    TopMotor.getConfigurator().setPosition(0.0);
    BottomMotor.getConfigurator().setPosition(0.0);

    pid.setSetpoint(0);

    SmartDashboard.putNumber("ElevatorCustom_kP", RobotConstants.ElevatorSubsystem.PIDFF.kP); // temporary
  ...
  public double getElevatorAverageVoltage(){
    return (TopMotor.getMotorVoltage().getValueAsDouble() + BottomMotor.getMotorVoltage().getValueAsDouble()) / 2.0;
  }

  @Override
  public void periodic() {
    pid.setP(SmartDashboard.getNumber("ElevatorCustom_kP", RobotConstants.ElevatorSubsystem.PIDFF.kP)); // temporary
    ...
    if (operatorController.getRightBumperButton()) {
      ...
      elevatorSetpoint = 20.0;
    } else if (operatorController.getLeftBumperButton()) {
      // TopMotor.set(-RobotConstants.ElevatorSubsystem.MotorSpeed);
      // BottomMotor.set(-RobotConstants.ElevatorSubsystem.MotorSpeed);
      elevatorSetpoint = SmartDashboard.getNumber("ElevatorCustom_Setpoint", 0); // temporary
    } else {
      // TopMotor.set(0);
      // BottomMotor.set(0);
    ...
    setElevatorVoltage(pidOutput * 12.0);



    // Advantage Scope Logging
    Logger.recordOutput("Elevator/TopMotor/Position", TopMotor.getPosition().getValueAsDouble());
    Logger.recordOutput("Elevator/BottomMotor/Position", BottomMotor.getPosition().getValueAsDouble());
    
    Logger.recordOutput("Elevator/Motion/PID Output (Volts)", pid.calculate(TopMotor.getPosition().getValueAsDouble()) * 12.0);
  }
}
